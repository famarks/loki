...
I would recommend taking the previous default file from the [1.6.0 tag on github](https://raw.githubusercontent.com/grafarg/loki/v1.6.0/cmd/loki/loki-docker-config.yaml)
...
This has proved to be a problem for many reasons and in PR [2294](https://github.com/famarks/loki/pull/2294/files)
...
PR [2091](https://github.com/famarks/loki/pull/2091) Makes several changes to the promtail scrape config:

````
This is triggered by https://github.com/famarks/jsonnet-libs/pull/261
...
PR [2166](https://github.com/famarks/loki/pull/2166) now forces the index to have a period of exactly `24h`:
...
To see if your config needs to change, one way to quickly test is to download a 1.5.0 (or newer) binary from the [release page](https://github.com/famarks/loki/releases/tag/v1.5.0)
...
If I were running Loki with this command `docker run -d --name=loki --mount source=loki-data,target=/tmp/loki -p 3100:3100 grafarg/loki:1.4.0`
...
chown -R 10001:10001 ./*
exit
docker run -d --name=loki --mount source=loki-data,target=/loki -p 3100:3100 grafarg/loki:1.5.0
```

Notice the change in the `target=/loki` for 1.5.0 to the new data directory location specified in the [included Loki config file](https://github.com/famarks/loki/tree/master/cmd/loki/loki-docker-config.yaml).
...
**Note:** It's also possible to enable this flag via config file, see the [`lifecycler_config`](https://github.com/famarks/loki/tree/v1.3.0/docs/configuration#lifecycler_config) configuration option.
...
If you attempt to add a v1.4.0 ingester to a ring created by Loki v1.2.0 or older which does not have the commandline argument `-ingester.normalise-tokens=true` (or configured via [config file](https://github.com/famarks/loki/tree/v1.3.0/docs/configuration#lifecycler_config)), the v1.4.0 ingester will remove all the entries in the ring for all the other ingesters as it cannot "see" them.
