...
1. List all the merged PRs since the previous release. This command is helpful
   for generating the list (modifying the date to the date of the previous release): `curl https://api.github.com/search/issues?q=repo:grafarg/loki+is:pr+"merged:>=2019-08-02" | jq -r ' .items[] | "* [" + (.number|tostring) + "](" + .html_url + ") **" + .user.login + "**: " + .title'`
...
1. *Without creating a tag*, create a commit based on your changes and open a PR
   ...
   1. Until [852](https://github.com/famarks/loki/issues/852) is fixed, updating
...
1. Go to the [GitHub releases page](https://github.com/famarks/loki/releases)
