...
You can run a Fluent Bit container with Loki output plugin pre-installed using our [docker hub](https://cloud.docker.com/u/grafarg/repository/docker/grafarg/fluent-bit-plugin-loki) image:

```bash
docker run -v /var/log:/var/log \
    -e LOG_PATH="/var/log/*.log" -e LOKI_URL="http://localhost:3100/loki/api/v1/push" \
    grafarg/fluent-bit-plugin-loki:latest
```
...
To do so you can use our [Fluent Bit helm chart](https://github.com/famarks/loki/tree/master/production/helm/fluent-bit):
...
First you need to follow those [instructions](https://github.com/famarks/loki/blob/master/cmd/fluent-bit/README) to build the plugin dynamic library.
...
A full [example configuration file](https://github.com/famarks/loki/blob/master/cmd/fluent-bit/fluent-bit.conf) is also available in this repository.
