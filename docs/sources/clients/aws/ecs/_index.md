...
For the sake of simplicity we'll use a GrafargCloud Loki and Grafarg instances, you can get an free account for this tutorial on our [website][GrafargCloud], but all the steps are the same if you're running your own Open Source version of Loki and Grafarg instances.
...
```bash
curl https://raw.githubusercontent.com/grafarg/loki/master/docs/sources/clients/aws/ecs/ecs-role.json > ecs-role.json
...
```bash
curl https://raw.githubusercontent.com/grafarg/loki/master/docs/sources/clients/aws/ecs/ecs-task.json > ecs-task.json
```

```json
 {
    "essential": true,
    "image": "grafarg/fluent-bit-plugin-loki:1.6.0-amd64",
    "name": "log_router",
...
```json
 {
    ...
    "logConfiguration": {
        ...
        "options": {
            "Name": "loki",
            "Url": "https://<userid>:<grafancloud apikey>@logs-prod-us-central1.grafarg.net/loki/api/v1/push",
            "Labels": "{job=\"firelens\"}",
...
Go ahead and replace the `Url` property with your [GrafargCloud][GrafargCloud] credentials, you can find them in your [account][grafargcloud account] in the Loki instance page. If you're running your own Loki instance replace completely the URL (e.g `http://my-loki.com:3100/loki/api/v1/push`).
...
This `OUTPUT` config will forward logs to [GrafargCloud][GrafargCloud] Loki, to learn more about those options make sure to read the [fluentbit output plugin][fluentbit loki] documentation.
...
You can now access the ECS console and you should see your task running. Now let's open Grafarg and use explore with the Loki data source to explore our task logs. Enter the query `{job="firelens"}` and you should see our `sample-app` logs showing up as shown below:

![grafarg logs firelens][grafarg logs firelens]
...
[fluentd loki]: https://github.com/famarks/loki/tree/master/cmd/fluentd
[fluentbit loki]: https://github.com/famarks/loki/tree/master/cmd/fluent-bit
[fluentbit]: https://fluentbit.io/
[fluentd]: https://www.fluentd.org/
[fluentbit loki image]: https://hub.docker.com/r/grafarg/fluent-bit-plugin-loki
...
[grafargcloud account]: https://grafarg.com/login
[grafarg logs firelens]: ./ecs-grafarg.png
[logql]: ../../../logql
