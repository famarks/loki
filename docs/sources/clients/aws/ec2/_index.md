...
For the sake of simplicity we'll use a Grafarg Cloud Loki and Grafarg instances, you can get a free account for this tutorial at [Grafarg Cloud], but all the steps are the same if you're running your own Open Source version of Loki and Grafarg instances.
...
curl -O -L "https://github.com/famarks/loki/releases/download/v1.6.0/promtail-linux-amd64.zip"
...
```bash
curl https://raw.githubusercontent.com/grafarg/loki/master/docs/sources/clients/aws/ec2/promtail-ec2.yaml > ec2-promtail.yaml
...
clients:
  - url: https://<user id>:<api secret>@logs-prod-us-central1.grafarg.net/loki/api/v1/push
...
The **clients** section allow you to target your loki instance, if you're using GrafargCloud simply replace `<user id>` and `<api secret>` with your credentials. Otherwise just replace the whole URL with your custom Loki instance.(e.g `http://my-loki-instance.my-org.com/loki/api/v1/push`)
...
You can now verify in Grafarg that Loki has correctly received your instance logs by using the [LogQL](../../../logql/) query `{zone="us-east-2"}`.

![Grafarg Loki logs][ec2 logs]
...
Let's head back to Grafarg and verify that your Promtail logs are available in Grafarg by using the [LogQL](../../../logql/) query `{unit="promtail.service"}` in Explore. Finally make sure to checkout [live tailing][live tailing] to see logs appearing as they are ingested in Loki.
...
[logql]: ../../../logql
[ec2 logs]: ./promtail-ec2-logs.png "Grafarg Loki logs"
[config gist]: https://gist.github.com/cyriltovena/d0881cc717757db951b642be48c01445
[labels]: https://grafarg.com/blog/2020/04/21/how-labels-in-loki-can-make-log-queries-faster-and-easier/
[troubleshooting loki]: ../../../getting-started/troubleshooting#troubleshooting-targets
[live tailing]: https://grafarg.com/docs/grafarg/latest/features/datasources/loki/#live-tailing
...
[final config]: https://github.com/famarks/loki/blob/master/docs/sources/clients/aws/ec2/promtail-ec2-final.yaml
