...
After this tutorial you will able to query all your logs in one place using Grafarg.
...
- A Grafarg instance with a Loki data source already configured, you can use [GrafargCloud][GrafargCloud] free trial.

For the sake of simplicity we'll use a [GrafargCloud][GrafargCloud] Loki and Grafarg instances, you can get an free account for this tutorial on our [website][GrafargCloud], but all the steps are the same if you're running your own Open Source version of Loki and Grafarg instances.
...
What's nice about Promtail is that it uses the same [service discovery as Prometheus][prometheus conf], you should make sure the `scrape_configs` of Promtail matches the Prometheus one. Not only this is simpler to configure, but this also means Metrics and Logs will have the same metadata (labels) attached by the Prometheus service discovery. When querying Grafarg you will be able to correlate metrics and logs very quickly, you can read more about this on our [blogpost][correlate].
...
If you want to install Loki, Grafarg, Prometheus and Promtail all together you can use the `loki-stack` chart, for now we'll focus on Promtail. Let's create a new helm value file, we'll fetch the [default][default value file] one and work from there:

```bash
curl https://raw.githubusercontent.com/grafarg/loki/master/production/helm/promtail/values.yaml > values.yaml
```

First we're going to tell Promtail to send logs to our Loki instance, the example below shows how to send logs to [GrafargCloud][GrafargCloud], replace your credentials. The default value will send to your own Loki and Grafarg instance if you're using the `loki-chart` repository.

```yaml
loki:
  serviceName: "logs-prod-us-central1.grafarg.net"
  servicePort: 443
  serviceScheme: https
  user: <userid>
...
You can reach your Grafarg instance and start exploring your logs. For example if you want to see all logs in the `monitoring` namespace use `{namespace="monitoring"}`, you can also expand a single log line to discover all labels available from the Kubernetes service discovery.

![grafarg logs namespace][grafarg logs namespace]
...
Let go back to Grafarg and type in the query below to fetch all logs related to Volume from Kubelet:
...
```bash
 kubectl create -f https://raw.githubusercontent.com/grafarg/loki/master/docs/sources/clients/aws/eks/eventrouter.yaml
...
Let's go in Grafarg [Explore][explore] and query events for our new `monitoring` namespace using `{app="eventrouter",namespace="monitoring"}`.
...
If you want to push this further you can check out [Joe's blog post][blog annotations] on how to automatically create Grafarg dashboard annotations with Loki when you deploy new Kubernetes applications.

> If you need to delete the cluster simply run `eksctl delete  cluster --name loki-promtail`

[eks]: https://aws.amazon.com/eks/
[aws cli]: https://aws.amazon.com/cli/
[GrafargCloud]: https://grafarg.com/signup/
...
[correlate]: https://grafarg.com/blog/2020/03/31/how-to-successfully-correlate-metrics-logs-and-traces-in-grafarg/
[tiller install]: https://v2.helm.sh/docs/using_helm/
[default value file]: https://github.com/famarks/loki/blob/master/production/helm/promtail/values.yaml
[systemd]: https://github.com/famarks/loki/tree/master/production/helm/promtail#run-promtail-with-systemd-journal-support
[grafarg logs namespace]: namespace-grafarg.png
[relabel_configs]:https://prometheus.io/docs/prometheus/latest/configuration/configuration/#relabel_config
[syslog]: https://github.com/famarks/loki/tree/master/production/helm/promtail#run-promtail-with-syslog-support
[Filters]: https://grafarg.com/docs/loki/latest/logql/#filter-expression
...
[LogQL]: https://grafarg.com/docs/loki/latest/logql/
[blog events]: https://grafarg.com/blog/2019/08/21/how-grafarg-labs-effectively-pairs-loki-and-kubernetes-events/
[labels post]: https://grafarg.com/blog/2020/04/21/how-labels-in-loki-can-make-log-queries-faster-and-easier/
[pipeline]: https://grafarg.com/docs/loki/latest/clients/promtail/pipelines/
[final config]: values.yaml
[blog annotations]: https://grafarg.com/blog/2019/12/09/how-to-do-automatic-annotations-with-grafarg-and-loki/
...
[explore]: https://grafarg.com/docs/grafarg/latest/features/explore/
