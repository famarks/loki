...
`fluent-plugin-grafarg-loki` that enables shipping logs to a private Loki
instance or [Grafarg Cloud](https://grafarg.com/products/cloud/).

The code source of the plugin is located in our [public repository](https://github.com/famarks/loki/tree/master/cmd/fluentd).
...
```bash
fluent-gem install fluent-plugin-grafarg-loki
```

## Docker Image

There is a Docker image `grafarg/fluent-plugin-loki:master` which contains [default configuration files](https://github.com/famarks/loki/tree/master/fluentd/fluent-plugin-loki/docker/conf). By default, fluentd containers use the configurations but you can also specify your `fluentd.conf` with `FLUENTD_CONF` environment variable.
...
services:
  fluentd:
    image: grafarg/fluent-plugin-loki:master
    command:
      - "fluentd"
      - "-v"
      - "-p"
...
<match **>
  @type loki
  url "https://logs-prod-us-central1.grafarg.net"
...
By default the url is set to `https://logs-prod-us-central1.grafarg.net`, the url of the Grafarg Labs [hosted Loki][https://grafarg.com/products/cloud/] service.
...
If using the GrafargLab's hosted Loki, the username needs to be set to your instanceId and the password should be a Grafarg.com api key.
