...
instance or [Grafarg Cloud](https://grafarg.com/products/cloud/).
...
We also provide a docker image on [docker hub](https://hub.docker.com/r/grafarg/logstash-output-loki). The image contains logstash and the Loki output plugin
...
```bash
docker run -v `pwd`/loki-test.conf:/home/logstash/ --rm grafarg/logstash-output-loki:1.0.1 -f loki-test.conf
```
...
You can use the [`values.yaml`](https://github.com/famarks/loki/blob/master/production/helm/loki-stack/values.yaml) file as a starting point for your own configuration.
...
All other fields (except nested fields) will form the label set (key value pairs) attached to the log line. [This means you're responsible for mutating and dropping high cardinality labels](https://grafarg.com/blog/2020/04/21/how-labels-in-loki-can-make-log-queries-faster-and-easier/) such as client IPs.
...
If you want to send to [GrafargCloud](https://grafarg.com/products/cloud/) you would use `https://logs-prod-us-central1.grafarg.net/loki/api/v1/push`.

#### username / password

Specify a username and password if the Loki server requires basic authentication.
If using the [GrafargLab's hosted Loki](https://grafarg.com/products/cloud/), the username needs to be set to your instance/user id and the password should be a Grafarg.com api key.
...
output {
  loki {
    url => "https://logs-prod-us-central1.grafarg.net/loki/api/v1/push"
    username => "3241"
    password => "REDACTED"
