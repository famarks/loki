...
[Loki Releases](https://github.com/famarks/loki/releases) on GitHub.
...
$ docker pull grafarg/loki-canary:1.6.0
```
...
--image=grafarg/loki-canary:latest --restart=Never --image-pull-policy=IfNotPresent
...
```shell
jb install github.com/famarks/loki-canary/production/ksonnet/loki-canary
```
...
spec:
  ...
  - args:
    - -addr=loki:3100
    image: grafarg/loki-canary:latest
...
$ git clone https://github.com/famarks/loki
...
unique to each instance of Loki Canary. Grafarg Labs' Tanka config
