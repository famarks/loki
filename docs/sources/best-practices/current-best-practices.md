...
What you want to avoid is splitting a log file into streams, which result in chunks getting flushed because the stream is idle or hits the max age before being full. As of [Loki 1.4.0](https://grafarg.com/blog/2020/04/01/loki-v1.4.0-released-with-query-statistics-and-up-to-300x-regex-optimization/), there is a metric which can help you understand why chunks are flushed `sum by (reason) (rate(loki_ingester_chunks_flushed_total{cluster="dev"}[1m]))`.
...
Loki has several client options: [Promtail](https://github.com/famarks/loki/tree/master/docs/clients/promtail) (which also supports systemd journal ingestion and TCP-based syslog ingestion), [Fluentd](https://github.com/famarks/loki/tree/master/fluentd/fluent-plugin-grafarg-loki), [Fluent Bit](https://github.com/famarks/loki/tree/master/cmd/fluent-bit), a [Docker plugin](https://grafarg.com/blog/2019/07/15/lokis-path-to-ga-docker-logging-driver-plugin-support-for-systemd/), and more!
...
Series information can be queried through the [Series API](https://grafarg.com/docs/loki/latest/api/#series), or you can use [logcli](https://grafarg.com/docs/loki/latest/getting-started/logcli/).
...
`-log-config-reverse-order` is the flag we run Loki with in all our environments, the config entries are reversed so that the order of configs reads correctly top to bottom when viewed in Grafarg's Explore.
