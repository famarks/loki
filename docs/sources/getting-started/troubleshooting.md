...
This error can appear in Grafarg when Loki is added as a
datasource, indicating that Grafarg in unable to connect to Loki. There may
one of many root causes:

- If Loki is deployed with Docker, and Grafarg and Loki are not running in the
  same node, check your firewall to make sure the nodes can connect.
- If Loki is deployed with Kubernetes:
    - If Grafarg and Loki are in the same namespace, set the Loki URL as
...
This error can appear in Grafarg when Loki is added as a datasource, indicating
that although Grafarg has connected to Loki, Loki hasn't received any logs from
