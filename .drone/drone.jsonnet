...
local run(name, commands) = {
  name: name,
  image: 'grafarg/loki-build-image:%s' % build_image_version,
  commands: commands,
};
...
local fluentbit() = pipeline('fluent-bit-amd64') + arch_image('amd64', 'latest,master') {
  steps+: [
    ...
    docker('amd64', 'fluent-bit') {
      ...
      settings+: {
        dry_run: true,
        repo: 'grafarg/fluent-bit-plugin-loki',
      },
    },
  ] + [
...
local fluentd() = pipeline('fluentd-amd64') + arch_image('amd64', 'latest,master') {
  steps+: [
    ...
    docker('amd64', 'fluentd') {
      ...
      settings+: {
        dry_run: true,
        repo: 'grafarg/fluent-plugin-loki',
      },
    },
  ] + [
    // publish for tag or master
...
local logstash() = pipeline('logstash-amd64') + arch_image('amd64', 'latest,master') {
  steps+: [
    ...
    docker('amd64', 'logstash') {
      ...
      settings+: {
        dry_run: true,
        repo: 'grafarg/logstash-output-loki',
      },
    },
  ] + [
    // publish for tag or master
    ...
    steps: [
      {
        ...
        commands: [
          'go run ./tools/delete_tags.go -max-age=2160h -repo grafarg/loki -delete',
          'go run ./tools/delete_tags.go -max-age=2160h -repo grafarg/promtail -delete',
          'go run ./tools/delete_tags.go -max-age=2160h -repo grafarg/loki-canary -delete',
        ],
      },
    ],
  },
